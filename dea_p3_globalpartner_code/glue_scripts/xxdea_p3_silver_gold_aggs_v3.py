...
def build_fact_order_item(order_items, options):
    opt_agg = options.groupBy("order_id", "lineitem_id").agg(
        F.sum(
            ...
            .otherwise(F.lit(0))
        ).cast("double").alias("discount_amount"),
        F.sum(
            F.when(F.col("option_price") > 0, F.col("option_price") * F.col("option_quantity"))
            .otherwise(F.lit(0))
        ).cast("double").alias("addon_amount"),
        F.sum(F.col("option_price") * F.col("option_quantity")).cast("double").alias("options_net_amount"),
    )
    ...
    # item_price arrives from Silver as decimal(10,2); every monetary column
    # from here on is cast to double explicitly so the physical Parquet type
    # always matches the Athena DDL (which declares these as double) - Spark's
    # decimal arithmetic otherwise keeps propagating DecimalType, which Athena
    # cannot read against a `double` column definition (HIVE_CURSOR_ERROR).
    fact = (
        fact.withColumn("item_price", F.col("item_price").cast("double"))
        .withColumn("item_gross_amount", (F.col("item_price") * F.col("item_quantity")).cast("double"))
        .withColumn("gross_revenue", (F.col("item_gross_amount") + F.col("addon_amount")).cast("double"))
        .withColumn("net_revenue", (F.col("item_gross_amount") + F.col("options_net_amount")).cast("double"))
...
def build_fact_order(fact_order_item):
    return fact_order_item.groupBy("order_id").agg(
        ...
        F.sum("gross_revenue").cast("double").alias("gross_revenue"),
        F.sum("discount_amount").cast("double").alias("discount_amount"),
        F.sum("net_revenue").cast("double").alias("net_revenue"),
...
def build_dim_customer(fact_order):
    return fact_order.groupBy("user_id").agg(
        ...
        F.sum("net_revenue").cast("double").alias("lifetime_net_revenue"),
...
# --------------------------------------------------------------------------
def build_fact_customer_daily(fact_order):
    ...
    daily_activity = fact_order.groupBy("user_id", "order_date").agg(
        F.sum("net_revenue").cast("double").alias("daily_net_revenue"),
    ...
    # Generate each customer's day sequence directly from their own order
    # history (first_order_date -> as_of_date) rather than joining against
    # date_dim's range. date_dim is a separately-sourced calendar table and
    # isn't guaranteed to cover the exact span of the order data; an inner
    # join against it silently produces zero rows on any mismatch, which is
    # exactly what happened here. sequence()/explode has no such dependency.
    customer_calendar = customer_bounds.withColumn(
        "snapshot_date",
        F.explode(F.sequence(F.col("first_order_date"), F.lit(as_of_date), F.expr("interval 1 day"))),
    ).select("user_id", "snapshot_date")

    # customer_calendar and daily_activity both trace back to the same
    # fact_order DataFrame (via separate groupBy calls), so an unqualified
    # join between them is an ambiguous self-join to Spark's analyzer.
    # Aliasing both sides explicitly disambiguates it.
    cal = customer_calendar.alias("cal")
    act = daily_activity.alias("act")

    joined = cal.join(
        act,
        (F.col("cal.user_id") == F.col("act.user_id"))
        & (F.col("cal.snapshot_date") == F.col("act.order_date")),
        "left",
    ).select(
        F.col("cal.user_id").alias("user_id"),
        F.col("cal.snapshot_date").alias("snapshot_date"),
        F.coalesce(F.col("act.daily_net_revenue"), F.lit(0)).alias("daily_net_revenue"),
        F.coalesce(F.col("act.daily_order_count"), F.lit(0)).alias("daily_order_count"),
    )
...
write_gold(build_fact_customer_daily(fact_order), "fact_customer_daily", partition_col="snapshot_date")
