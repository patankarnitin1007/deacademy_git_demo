...
def quarantine_reason(*checks):
    """checks: list of (condition_is_bad, message) tuples.

    concat_ws skips NULL inputs by design, so this yields '' when no
    check is flagged and a "; "-joined list of reasons otherwise. This
    avoids array_remove(array, None), which can return a NULL array
    (rather than nulls-stripped) depending on how the plan evaluates it -
    and a NULL array's size() is -1, which matches neither > 0 nor == 0,
    silently dropping rows out of both the valid and quarantine filters.
    """
...
def split_valid_invalid(df):
    invalid = df.filter(F.length("_reject_reason") > 0)
    valid = df.filter(F.length("_reject_reason") == 0).drop("_reject_reason")
